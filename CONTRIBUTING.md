...
This command compiles the `main.go` file and places the `format-yaml` executable in `$XDG_CACHE_HOME/go/bin`.

Alternatively, without `just`, you can use `go build`:

```bash
go build -o "$(go env GOCACHE)/bin/format-yaml" .
```

Ensure that `$XDG_CACHE_HOME/go/bin` (or `$(go env GOCACHE)/bin`) is included in your system's `PATH` environment
variable to run `format-yaml` from any directory.
